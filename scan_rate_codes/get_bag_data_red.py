...
count(bag_seal_number) as c \
FROM %s \
WHERE scanned_datetime between '%s' and '%s' \
...
fout = csv.writer(open('bag_facility_list.csv','w'))
...
df_all_fac.to_csv('/disk1/hdf_data/facility_wise_hourly_bag_scan_agg.csv', index = False)
