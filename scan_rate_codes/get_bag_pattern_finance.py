...
store =pd.HDFStore('july_bag_1_7_16_scans.h5')
...
#    print fac
 ...
 #       df_sorted = df_.sort(['sd'])
        df_fac['sd_day'] = df_fac.apply(get_day, axis = 1)
        df_fac['sd_hour'] = df_fac.apply(get_hour, axis = 1)
        
        df_grp = df_fac.groupby(['sd_day','sd_hour']).bs.count().reset_index()
#        columns = []
#        for i in range(1,7):
#            d = 'day_%s' % i
#            columns.append(d)
#            index = range(24)
        ...
        print fac
df_all_fac.to_csv('bag_test.csv', index = False)
